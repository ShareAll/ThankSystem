...
public abstract class AbstractDao<T> {
	...
	public AbstractDao(MongoClient client, String dbName, Class<T> cls) {
		...
		morphia.map(cls);
		if (dbName == null) {
			dbName = (String) properties
					.getProperty(MONGO_DB, DEFAULT_MONGO_DB);
		}
		if (client == null) {
	...
	public MongoClient getMongoClient(MongoClient mongoClient,
			Properties properties) {
		properties = properties == null ? this.properties : properties;
		try {
			if (mongoClient == null) {
				int port = Integer.valueOf((String) properties.get(MONGO_PORT));
				ServerAddress sd = new ServerAddress(
						(String) properties.get(MOMGO_HOST_NAME), port);
				if (properties.get(MONGO_USER) != null) {
					MongoCredential credential = MongoCredential
							.createMongoCRCredential((String) properties
									.get(MONGO_USER), (String) properties
									.get(MONGO_DB), ((String) properties
									.get(MONGO_PWD)).toCharArray());
	...
	public static Properties loadProperties() {
		...
		try {
			...
			inputStream = AbstractDao.class.getClassLoader()
